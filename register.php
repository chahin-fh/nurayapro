...
<body>
    ...
    <script>
        function togglePasswordVisibility(fieldId) {
            try {
                ...
                if (!passwordInput || !passwordIcon) {
                    console.error('Elements not found:', { fieldId, passwordInput, passwordIcon });
                    return;
                }
        ...
        function verifyEmail(email) {
            return fetch('src/Controllers/api/auth_hybrid.php', {
                method: 'POST',
                headers: {
                ...
                verifyEmail(email).then(verifyResult => {
                    if (!verifyResult.success) {
                        if (verifyResult.message && verifyResult.message.includes('existe déjà')) {
                            showAlert('Cet email est déjà utilisé. Veuillez vous connecter.', 'error');
                        } else {
                            showAlert(verifyResult.message || 'Erreur de vérification email', 'error');
                        }
                    ...
                    sendVerificationCode(email, firstName, lastName).then(sendResult => {
                        if (!sendResult.success) {
                            showAlert(sendResult.message || 'Erreur d\'envoi du code de vérification', 'error');
                            registerBtn.disabled = false;
                            registerBtn.textContent = 'S\'inscrire';
                            return;
                        }
                        ...
                        showAlert(`Un code de vérification a été envoyé à ${email}. Redirection...`, 'success');
                    ...
                    if (email && email.includes('@') && email.includes('.')) {
                        ...
                        emailVerificationTimeout = setTimeout(() => {
                            verifyEmail(email).then(result => {
                                if (!result.success && result.message && result.message.includes('existe déjà')) {
