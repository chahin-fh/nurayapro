...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Desktop search input event listeners
        if (searchInput) {
        ...
        if (mobileSearchInput) {
            ...
            mobileSearchInput.addEventListener('keypress', function (e) {
                if (e.key === 'Enter') {
                    const searchTerm = this.value.trim();
                    if (searchTerm) {
                        window.location.href = `${basePath}shop.php?search=${encodeURIComponent(searchTerm)}`;
                    }
                }
            });
        }
    ...
    function handleMobileSearchInput() {
        ...
        searchTimeout = setTimeout(() => {
            fetchSearchResultsMobile(searchTerm);
        }, 300);
    }

    function fetchSearchResultsMobile(searchTerm) {
        fetch(`${basePath}api/search.php?q=${encodeURIComponent(searchTerm)}`)
            .then(response => response.json())
            .then(data => {
    ...
    function displaySearchResultsMobile(data) {
        const resultsContainer = document.getElementById('mobileSearchResults');
        ...
        if (data.success && data.results.length > 0) {
            ...
            data.results.forEach(product => {
                ...
                <a href="${basePath}product.php?id=${product.product_id}" class="search-result-item">
                    <img src="${product.image_url}" alt="${product.name}" class="search-result-image"
                        onerror="this.src='https://via.placeholder.com/40x40/F5EFE6/C8B6A6?text=Produit'">
                    <div class="search-result-info">
                        <div class="search-result-name">${product.name}</div>
                        <div class="search-result-category">${product.category_name}</div>
                    </div>
